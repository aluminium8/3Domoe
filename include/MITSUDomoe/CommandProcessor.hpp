...
public:
    ...
    void register_cartridge(const std::string &command_name, C cartridge)
    {
        ...
        // 1. カートリッジのInput型のデフォルトインスタンスを作成
        typename C::Input default_input;
        // 2. それをjson形式にシリアライズ
        const auto default_json = rfl::json::write(default_input);
        std::cout<<"default: "<<default_json<<std::endl;
